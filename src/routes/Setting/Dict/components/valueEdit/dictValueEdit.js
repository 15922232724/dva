...
export default class extends BaseComponent {
  ...
  componentDidMount() {
    ...
    if (state) {
      const id = state.id;
      const dictTypeId = state.dictTypeId;
      this.setState({
        dictTypeId
      });
      if(id){
        ...
        dispatch(actions.getDictValue({ id }))
      }
    }
  }
  ...
  handlerSave(value) {
    ...
    dispatch(actions.saveDictValue({
      value,
      success: () => {
        this.setState({
          record: null
  ...
  handlerUpdate(value) {
    const { dispatch, history } = this.props;
    dispatch(actions.updateDictValue({
      value,
      success: () => {
        this.setState({
          record: null,
  ...
  render() {
    ...
    const { loading, saveLoading, history, dict: { dictValue } } = this.props;
    let { record,dictTypeId } = this.state;
    const title = (record ? '字典值编辑' : '字典值新增');
    /**
     * 新增、修改都会进到这个方法中，
